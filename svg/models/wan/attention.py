...
from svg.kernels.triton.pisa_attention import piecewise_sparse_attention, semantic_piecewise_attention
from svg.kernels.triton.mix_pisa_attention import ml_pisa_attention
from svg.kernels.triton.group_pisa_attention import pisa_pg_attention

from ...kmeans_utils import compute_centroids

from svg.rectify import rectified_sparse_attention
...
from ...kmeans_utils import (
    ...
    identify_triple_dynamic_map_and_probs,
    identify_fine_triple_map_and_probs,
)
...
class WanAttn_SAPAttn_Processor(WanAttn_SVGAttn_Processor2_0):
    ...
    stability_tracker: Optional[ClusterStabilityTracker] = None  # 稳定性分析跟踪器
    ...
    def kmeans_init(self, query, key, layer_idx):
        ...
        qlabels, qcentroids, qcluster_sizes, qiter = batch_kmeans_Euclid(
            query.view(cfg * num_heads, seq_len, dim), n_clusters=self.num_q_centroids, max_iters=self.q_kmeans_iter_init, init_method=self.q_kmeans_init_method
        )
        klabels, kcentroids, kcluster_sizes, kiter = batch_kmeans_Euclid(
            key.view(cfg * num_heads, seq_len, dim), n_clusters=self.num_k_centroids, max_iters=self.k_kmeans_iter_init, init_method=self.k_kmeans_init_method
        )
    ...
    @time_logging_decorator("Level 3.7 - kmeans step (key only)")
    def kmeans_step_key_only(self, query, key, layer_idx):
        """只对 key 做进一步聚类，query 仅按当前 q_centroids 分配，不更新 q 的聚类中心。"""
        cfg, num_heads, seq_len, dim = query.size()
        flat = cfg * num_heads

        # Q: 仅分配，不迭代更新中心
        query_flat = query.view(flat, seq_len, dim)
        q_sq = (query_flat**2).sum(dim=-1)
        qlabels = euclid_assign_triton(query_flat, self.q_centroids, q_sq)
        qcentroids = self.q_centroids
        ones = torch.ones_like(qlabels, dtype=torch.int64, device=query.device)
        qcluster_sizes = torch.zeros(
            flat, self.num_q_centroids, dtype=torch.int64, device=query.device
        )
        qcluster_sizes.scatter_add_(1, qlabels, ones)
        qiter = 0

        # K: 与 kmeans_step 相同，做进一步聚类
        klabels, kcentroids, kcluster_sizes, kiter = batch_kmeans_Euclid(
            key.view(flat, seq_len, dim),
            n_clusters=self.num_k_centroids,
            max_iters=self.k_kmeans_iter_step,
            init_centroids=self.k_centroids,
        )
    ...
    def kmeans_clustering(self, query, key, layer_idx):
        if not self.centroids_init:
            ...
            print(f"Centroids initialized at layer {layer_idx}. Q init step: {self.q_kmeans_iter_init}, K init step: {self.k_kmeans_iter_init}")
        else:
            step_fn = self.kmeans_step_key_only if self.kmeans_step_type == "key_only" else self.kmeans_step
            qlabels, qcentroids, qcluster_sizes, qiter, klabels, kcentroids, kcluster_sizes, kiter = step_fn(
                query, key, layer_idx
            )
    ...
    def attention_core_logic(self, query, key, value, timestep):
        ...
        # print(f"timestep: {timestep[0]}, layer: {self.layer_idx}, full_attention_flag: {full_attention_flag}")
        ...
        if full_attention_flag:
            ...
            # Record stability data if tracker is enabled
            # if self.stability_tracker is not None and self.q_centroids is not None and self.k_centroids is not None:
            #     timestep_val = timestep[0].item() if isinstance(timestep, torch.Tensor) else timestep
            #     self.stability_tracker.record_timestep(
            ...
            #     )
            #     # 立即生成与上一个 step 的对比可视化
            #     save_path = self.stability_tracker.visualize_step_comparison(timestep_val, self.layer_idx)
            #     if save_path:
            #         print(f"[Stability] Step {timestep_val} comparison saved: {save_path}")
            # if self.stability_tracker is not None and self.q_centroids is not None and self.k_centroids is not None:
            #     timestep_val = timestep[0].item() if isinstance(timestep, torch.Tensor) else timestep
            #     self.stability_tracker.record_timestep(
            #         timestep=timestep_val,
            ...
            attn_output = attn_output.reshape(cfg, num_heads, seq_len, dim)

            # Save q, k, v, output when timestep==940 and layer_idx==3 for debugging
            timestep_val = timestep[0].item() if isinstance(timestep, torch.Tensor) else timestep
            if timestep_val == 940 and self.layer_idx == 3:
                save_dir = "/root/autodl-tmp/tensor_ckpts"
                os.makedirs(save_dir, exist_ok=True)
                torch.save(attn_output.detach().cpu(), os.path.join(save_dir, "sap_output.pt"))
                logger.info(f"Saved sap output to {save_dir} (timestep=940, layer={self.layer_idx})")
                assert False

            return attn_output


class WanAttn_SAPAttn_Processor_SR(WanAttn_SAPAttn_Processor):
    centroids_cache_dir = "centroids_cache"  # Directory to save/load centroids cache

    def _get_centroids_cache_path(self, layer_idx, is_query=True):
        """Get the cache file path for centroids."""
        os.makedirs(self.centroids_cache_dir, exist_ok=True)
        prefix = "q" if is_query else "k"
        num_centroids = self.num_q_centroids if is_query else self.num_k_centroids
        filename = f"{prefix}centroids_layer_{layer_idx}_n_{num_centroids}.pt"
        return os.path.join(self.centroids_cache_dir, filename)

    def _load_centroids_from_cache(self, layer_idx, is_query=True, device=None):
        """Load centroids from cache file."""
        cache_path = self._get_centroids_cache_path(layer_idx, is_query)
        if os.path.exists(cache_path):
            centroids = torch.load(cache_path, map_location=device)
            print(f"Loaded {'query' if is_query else 'key'} centroids from cache: {cache_path}")
            return centroids
        else:
            print(f"Cache file not found: {cache_path}")
            return None

    def _save_centroids_to_cache(self, centroids, layer_idx, is_query=True):
        """Save centroids to cache file."""
        cache_path = self._get_centroids_cache_path(layer_idx, is_query)
        torch.save(centroids, cache_path)
        print(f"Saved {'query' if is_query else 'key'} centroids to cache: {cache_path}")
    ...
    def kmeans_init(self, query, key, layer_idx):
        ...
        # Q: Use cache if q_kmeans_init_method is "cache", otherwise use q_kmeans_init_method
        q_init_centroids = None
        if self.q_kmeans_init_method == "cache":
            q_init_centroids = self._load_centroids_from_cache(layer_idx, is_query=True, device=device)
            if q_init_centroids is None:
                init_method_q = "random"  # Fallback to random if cache not found
            else:
                init_method_q = "cache"
        else:
            init_method_q = self.q_kmeans_init_method
        
        # K: Use k_kmeans_init_method
        init_method_k = self.k_kmeans_init_method
        
        query_flat = query.view(cfg * num_heads, seq_len, dim)
        key_flat = key.view(cfg * num_heads, seq_len, dim)
        
        if q_init_centroids is not None:
            qlabels, qcentroids, qcluster_sizes, qiter = batch_kmeans_Euclid(
                query_flat, n_clusters=self.num_q_centroids, max_iters=self.q_kmeans_iter_init, init_centroids=q_init_centroids
            )
        else:
            qlabels, qcentroids, qcluster_sizes, qiter = batch_kmeans_Euclid(
                query_flat, n_clusters=self.num_q_centroids, max_iters=self.q_kmeans_iter_init, init_method=init_method_q
            )
            if self.q_kmeans_init_method == "cache":
                self._save_centroids_to_cache(qcentroids, layer_idx, is_query=True)
        
        klabels, kcentroids, kcluster_sizes, kiter = batch_kmeans_Euclid(
            key_flat, n_clusters=self.num_k_centroids, max_iters=self.k_kmeans_iter_init, init_method=init_method_k
        )
    ...
    def attention_core_logic(self, query, key, value, timestep):
        ...
        if full_attention_flag:
            ...
            q_perm, k_perm, v_perm, qcluster_sizes, kcluster_sizes, q_sorted_indices, qlabels, klabels, qcentroids, kcentroids, triple_dynamic_map, weighted_attn_probs = self.sr_semantic_aware_permutation(
                query, key, value
            )

            output_permuted = rectified_sparse_attention(
                q_perm, k_perm, v_perm, 
                qlabels, qcentroids, qcluster_sizes, 
                klabels, kcentroids, kcluster_sizes,
                triple_dynamic_map,
                weighted_attn_probs,
                self.num_q_centroids, self.num_k_centroids,
    ...
    @time_logging_decorator("Level 3 - semantic aware permutation for SR attention")
    def sr_semantic_aware_permutation(self, query, key, value):
        """
        Returns:
            q_permuted: Permuted query tensor, [B, H, S, D]
            k_permuted: Permuted key tensor, [B, H, S, D]
            v_permuted: Permuted value tensor, [B, H, S, D]
            q_cluster_sizes: Cluster sizes of query tensor, [B, H, num_q_centroids]
            k_cluster_sizes: Cluster sizes of key tensor, [B, H, num_k_centroids]
            q_sorted_indices: Sorted indices of query tensor, [B, H, S]
            qlabels: Labels of query tensor, [B, H, S]
            klabels: Labels of key tensor, [B, H, S]
            qcentroids: Centroids of query tensor, [B, H, num_q_centroids, D]
            kcentroids: Centroids of key tensor, [B, H, num_k_centroids, D]
        """
        cfg, num_heads, seq_len, dim = query.size()

        assert cfg == 1, "Batch size must be 1 for semantic aware permutation for SR attention"

        # 1. Kmeans clustering
        qlabels, q_centroids, q_cluster_sizes, qiter, klabels, k_centroids, k_cluster_sizes, kiter = self.kmeans_clustering(
            query, key, self.layer_idx
        )

        # 2. Identify dynamic map
        q_cluster_sizes = q_cluster_sizes.view(cfg, num_heads, self.num_q_centroids)
        k_cluster_sizes = k_cluster_sizes.view(cfg, num_heads, self.num_k_centroids)
        q_centroids = q_centroids.view(cfg, num_heads, self.num_q_centroids, dim)
        k_centroids = k_centroids.view(cfg, num_heads, self.num_k_centroids, dim)

        triple_dynamic_map, weighted_attn_probs = identify_fine_triple_map_and_probs(
            q_centroids,
            k_centroids,
            q_cluster_sizes,
            k_cluster_sizes,
        ...
        q_labels = qlabels.view(cfg, num_heads, seq_len)
        k_labels = klabels.view(cfg, num_heads, seq_len)

        return q_permuted, k_permuted, v_permuted, q_cluster_sizes, k_cluster_sizes, q_sorted_indices, q_labels, k_labels, q_centroids, k_centroids, triple_dynamic_map, weighted_attn_probs

class WanAttn_PISAttn_Processor(WanAttn_SAPAttn_Processor):

    def kv_permutation(self, key, value):
        """Permute key/value by klabels (from k-means on key)."""
        k_permuted, k_sorted_indices = permute_tensor_by_labels_triton(key, self.klabels, dim=2)
        v_permuted, _ = permute_tensor_by_labels_triton(
            value, self.klabels, dim=2, sorted_indices=k_sorted_indices
        )

        return k_permuted, v_permuted
    
    def key_kmeans_init(self, key, layer_idx):
        cfg, num_heads, seq_len, dim = key.size()
        klabels, kcentroids, kcluster_sizes, kiter = batch_kmeans_Euclid(
            key.view(cfg * num_heads, seq_len, dim),
            n_clusters=self.num_k_centroids,
            max_iters=self.k_kmeans_iter_init,
            init_method=self.k_kmeans_init_method
        )
        self.k_centroids = kcentroids
        return klabels, kcentroids, kcluster_sizes, kiter
    
    def key_kmeans_step(self, key, layer_idx):
        cfg, num_heads, seq_len, dim = key.size()
        klabels, kcentroids, kcluster_sizes, kiter = batch_kmeans_Euclid(
            key.view(cfg * num_heads, seq_len, dim),
            n_clusters=self.num_k_centroids,
    ...
    @time_logging_decorator("Level 3.7 - kmeans clustering")
    def key_kmeans_clustering(self, key, layer_idx):
        if not self.centroids_init:
            klabels, kcentroids, kcluster_sizes, kiter = self.key_kmeans_init(
                key, layer_idx
            )
            self.centroids_init = True
        else:
            klabels, kcentroids, kcluster_sizes, kiter = self.key_kmeans_step(
                key, layer_idx
            )
    ...
    def attention_core_logic(self, query, key, value, timestep):
        ...
        if full_attention_flag:
            ...
            qlabels, q_centroids, q_cluster_sizes, _, klabels, k_centroids, k_cluster_sizes, _ = self.kmeans_clustering(query, key, self.layer_idx)
            
            self.klabels = klabels.view(cfg, num_heads, seq_len)

            k_cluster_sizes = k_cluster_sizes.view(cfg, num_heads, self.num_k_centroids)
            k_centroids = k_centroids.view(cfg, num_heads, self.num_k_centroids, dim)

            # Permute q/k/v by qlabels/klabels, then inverse-permute output by q permutation.
            q_perm, q_sorted_indices = permute_tensor_by_labels_triton(query, qlabels, dim=2)
            k_perm, k_sorted_indices = permute_tensor_by_labels_triton(key, klabels, dim=2)
            v_perm, _ = permute_tensor_by_labels_triton(value, klabels, dim=2, sorted_indices=k_sorted_indices)

            output_permuted = semantic_piecewise_attention(
                q_perm, k_perm, v_perm,
                k_cluster_sizes, k_centroids, block_size=16
            )
            attn_output = apply_inverse_permutation_triton(output_permuted, q_sorted_indices, dim=2)
            attn_output = attn_output.reshape(cfg, num_heads, seq_len, dim)
...
class Wan_PISA_Processor(WanAttn_SVGAttn_Processor2_0):
    def __init__(self, layer_idx):
    ...
    def attention_core_logic(self, query, key, value, timestep):
        ...
        if full_attention_flag:
            ...
            output = piecewise_sparse_attention(
                query, key, value,
                density=0.2, block_size=64
            )

            return output.reshape(cfg, num_heads, seq_len, dim)


class Wan_MIX_PISA_Processor(WanAttn_SVGAttn_Processor2_0):
    def __init__(self, layer_idx):
        super().__init__(layer_idx)
        self.layer_idx = layer_idx
...
class Wan_GROUP_PISA_Processor(WanAttn_SVGAttn_Processor2_0):
    def __init__(self, layer_idx):
        super().__init__(layer_idx)
        self.layer_idx = layer_idx
        self.rescaled_density = None  # setup 时按 layer 填入 {str(timestep): float}
    ...
    def attention_core_logic(self, query, key, value, timestep):
        ...
        ts_key = str(int(timestep[0].item()))
