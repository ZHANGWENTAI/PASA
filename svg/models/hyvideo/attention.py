...
from ...kernels.triton.pisa_attention import piecewise_sparse_attention
from ...kernels.triton.group_pisa_attention import pisa_pg_attention
from ...kmeans_utils import (
    batch_kmeans_Euclid,
    density_calculation,
    dynamic_block_sparse_fwd_flashinfer,
    identify_dynamic_map,
    identify_dynamic_map_and_probs,
)
from svg.rectify import rectified_sparse_attention
from ...logger import logger
from .utils import lookup_rescaled_density
...
class Hunyuan_SAPAttn_Processor2_0_SR(Hunyuan_SAPAttn_Processor2_0):
    centroids_cache_dir = "centroids_cache"
    ...
    def kmeans_init(self, query, key, layer_idx):
        ...
        if q_init_centroids is not None:
            qlabels, qcentroids, qcluster_sizes, qiter = batch_kmeans_Euclid(
                query_flat, n_clusters=self.num_q_centroids, max_iters=self.q_kmeans_iter_init, verbose=True, init_centroids=q_init_centroids
            )
        else:
            qlabels, qcentroids, qcluster_sizes, qiter = batch_kmeans_Euclid(
                query_flat, n_clusters=self.num_q_centroids, max_iters=self.q_kmeans_iter_init, verbose=True, init_method=init_method_q
            )
            if self.q_kmeans_init_method == "cache":
                self._save_centroids_to_cache(qcentroids, layer_idx, is_query=True)
        
        klabels, kcentroids, kcluster_sizes, kiter = batch_kmeans_Euclid(
            key_flat, n_clusters=self.num_k_centroids, max_iters=self.k_kmeans_iter_init, verbose=True, init_method=init_method_k
        )
    ...
    def sr_semantic_aware_permutation(self, query, key, value, timestep, layer_idx):
        """
        Returns:
        ...
        q_cluster_sizes = qcluster_sizes.view(cfg, num_heads, self.num_q_centroids)
        k_cluster_sizes = kcluster_sizes.view(cfg, num_heads, self.num_k_centroids)
        q_centroids = qcentroids.view(cfg, num_heads, self.num_q_centroids, dim)
        k_centroids = kcentroids.view(cfg, num_heads, self.num_k_centroids, dim)

        dynamic_map, weighted_attn_probs = identify_dynamic_map_and_probs(
            q_centroids,
            k_centroids,
            q_cluster_sizes,
            k_cluster_sizes,
        ...
        return q_permuted, k_permuted, v_permuted, q_cluster_sizes, k_cluster_sizes, q_sorted_indices.unsqueeze(0), qlabels.unsqueeze(0), klabels.unsqueeze(0), qcentroids.unsqueeze(0), kcentroids.unsqueeze(0), dynamic_map, weighted_attn_probs
    ...
    def attention_core_logic(self, query, key, value, timestep, layer_idx, cu_max_seqlens):
        ...
        if full_attention_flag:
            ...
            q_perm, k_perm, v_perm, q_cluster_sizes, k_cluster_sizes, q_sorted_indices, qlabels, klabels, qcentroids, kcentroids, dynamic_map, weighted_attn_probs = self.sr_semantic_aware_permutation(
                query_video, key_video, value_video, timestep, layer_idx
            )

            output_permuted = rectified_sparse_attention(
                q_perm, k_perm, v_perm, 
                qlabels, qcentroids, q_cluster_sizes, 
                klabels, kcentroids, k_cluster_sizes,
                dynamic_map,
                weighted_attn_probs,
            ...
            attn_output[:, :, :video_length, :] = apply_inverse_permutation_triton(output_permuted, q_sorted_indices, dim=2)

            # Save time, layer, density information to logging file
            if self.logging_file is not None:
                with time_logging_decorator("Level 3 - density calculation and logging"):
                    # 4. Calculate density
                    densities = density_calculation(dynamic_map, q_cluster_sizes, k_cluster_sizes)
...
class Hunyuan_PISA_Processor(Hunyuan_SAPAttn_Processor2_0):
    ...
    def attention_core_logic(self, query, key, value, timestep, layer_idx, cu_max_seqlens):
        ...
        if full_attention_flag:
            ...
            ts = int(timestep[0].item())
            density = lookup_rescaled_density(self.rescaled_density, ts)
            assert density is not None, f"Density is not found for timestep {ts}"

            output_hidden_states = piecewise_sparse_attention(query, key, value, density=density, block_size=64, use_random=self.use_random)
            return output_hidden_states.reshape(cfg, num_heads, seq_len, dim)


class Hunyuan_PASA_Processor(Hunyuan_SAPAttn_Processor2_0):
    ...
    def attention_core_logic(self, query, key, value, timestep, layer_idx, cu_max_seqlens):
        ...
        if full_attention_flag:
            ...
            if self.use_group:
                attn_output = pisa_pg_attention(query, key, value, density=density, block_size=64, use_random=self.use_random)
            else:
                attn_output = piecewise_sparse_attention(query, key, value, density=density, block_size=64, use_random=self.use_random)
